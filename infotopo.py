...
##############################################################################
    ...
    def simplicial_interact_info_decomposition(self, Nentropie_input):
        N_interact_info={}
        for x,y in Nentropie_input.items():
            for k in range(1, len(x)+1):
                for subset in itertools.combinations(x, k):
                    N_interact_info[x]=N_interact_info.get(x,0)+ ((-1)**(len(x) - len(subset)))*Nentropie_input[subset]             
        return (N_interact_info)    
...
# Nentropy is dictionary (x,y) with x a list of kind (1,2,5) and y a value in bit    
    ...
    information_topo = infotopo(dimension_max = dimension_max, 
                                ...
                                supervised_mode = supervised_mode, 
                                forward_computation_mode = forward_computation_mode,
                                dim_to_rank = 4, number_of_max_val = 2)
    if dataset_type != 5:
...
# Ninfomut is a dictionary (x,y) with x a list of kind (1,2,5) and y a value in bit
    ...
    N_interact_info = information_topo.simplicial_interact_info_decomposition(Nentropie)
    information_topo.mutual_info_simplicial_lanscape(N_interact_info)
    if dataset_type != 5: 
        dico_max, dico_min = information_topo.display_higher_lower_information(N_interact_info, dataset)
    adjacency_matrix_mut_info = information_topo.mutual_info_pairwise_network(N_interact_info)
    mean_info, mean_info_rate  =information_topo.display_mean_information(N_interact_info)
